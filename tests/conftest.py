...
def load():
    ...
    tpa1 = ThirdPartyApp.objects.create(user=User.objects.get(id=1), name='webapp', secret=make_password(password), 
                description='tpa1', enabled=True, redirect_uris='')

    tpa2 = ThirdPartyApp.objects.create(user=User.objects.get(id=1), name='tpa2', secret=make_password(password), 
